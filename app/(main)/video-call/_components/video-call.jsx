...
export default function VideoCall({ sessionId, token }) {
  ...
  // Initialize Vonage video session
  const initializeSession = () => {
    if (!appId || !sessionId || !token) {
      toast.error("Missing video call parameters");
    ...
    try {
      ...
      session.on("streamCreated", (event) => {
        session.subscribe(
          ...
          (error) => {
            if (error) toast.error("Error subscribing to stream");
          }
        );
      });
          ...
          <div className="space-y-6">
            ...
            <div className="text-center">
              ...
              <p className="text-muted-foreground text-sm mt-1">
                When you're finished, click the red button to end the call.
              </p>
            </div>
          </div>
        )}
